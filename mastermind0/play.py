...
def play_log(codemaker, codebreaker, name):
   
   with open(name + '.txt', 'w') as log:    # ouvre un fichier texte appelé log 
    ...
    while True:
        combinaison = codebreaker.codebreaker(ev)
        ev = codemaker.codemaker(combinaison)
        log.write(combinaison + '\n' + str(ev[0]) +"," + str(ev[1]) + '\n')  #  écrit dans le fichier texte la combinaison testée et son évaluation à la ligne

        if ev[0] >= common.LENGTH:
            print (log)    # affiche le fichier texte log
