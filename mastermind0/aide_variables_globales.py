...
f()
print(x)  # Erreur ("name 'x' is not defined") : la variable x est locale �  la fonction f, c'est �  dire qu'elle n'existe pas en dehors de cette fonction
...
f()
g()  # Erreur ("name 'x' is not defined") : pareil que ci-dessus, la variable x est locale �  la fonction f
...
# Conclusion des exemples ci-dessus : par défaut, les variables définies "dans le main" (i.e. en dehors des fonctions) peuvent utilisées *en lecture seulement* par des fonctions (ce sont des variables *globales*). En revanche, par défaut lorsqu'une fonction fait une affectation sur une variable, celle-ci devient automatiquement *locale* �  la fonction (et l'accès en lecture par cette fonction �  la valeur de la variable définie en dehors de la fonction n'est alors pas possible : ce ne sont en fait plus les mêmes variables).
